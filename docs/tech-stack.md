...
4. 先锁住接口契约和数据模型，再锁实现它们所需的最小技术路线。
5. 不恢复旧实现里的技术假设；若旧实现与当前文档冲突，以当前文档为准。
...
- `Web` 可在后续阶段再补回
- 先保留 `Central`、`Host`、`shared` 三块最小结构
