...
本文档定义当前 docs-only baseline 下，Agent Mail email-oriented POC 的实现计划草案。
...
1. 先锁规格，再开始写代码。
2. 先做 `Central` 数据与接口，再做 `Host` 和 `MCP`。
3. 先打通一条最小闭环，再补调试与观测能力。
4. 先让一封邮件跑通，再扩大到更多角色和更多边界场景。
5. 不恢复旧实现路径；如果旧代码与当前文档冲突，以当前文档为准。
...
从当前 docs-only baseline 重新搭建代码仓库骨架。
...
完成该实现计划后，仓库应从 docs-only baseline 进入一个新的、与当前规范完全一致的代码基线：
